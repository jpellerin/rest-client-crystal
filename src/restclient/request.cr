...
module RestClient
  ...
  class Request
    ...
    def initialize args
      if not args[:method]
        raise ArgumentError.new("must pass :method")
      end
      @method = args[:method]
      @headers = (args[:headers] || {} of String => String).dup
      if args[:url]
        @url = process_url_params(args[:url], headers)
      else
        raise ArgumentError, "must pass :url"
      end
      @cookies = @headers.delete(:cookies) || args[:cookies] || {} of String => String
    ...
    def process_url_params url, headers
      url_params = {} of String => (String|Hash)
    ...
    def self.default_ssl_cert_store
      ...
      if RestClient::Platform.windows?
        RestClient::Windows::RootCerts.instance.to_a.uniq.each do |cert|
          ...
          rescue err : OpenSSL::X509::StoreError
    ...
    rescue err : Timeout::Error  # FIXME need also | Errno::ETIMEDOUT)
    ...
    rescue errpr : OpenSSL::SSL::SSLError
    ...
    def stringify_headers headers
      headers.inject({} of String => String) do |result, key, value| # XXX
        if key.is_a? Symbol
    ...
    # @return [String]
    #
    private def maybe_convert_extension(ext)
